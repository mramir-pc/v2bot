...
telegramBotToken=$(cat /var/www/html/v2bot/baseInfo.php | grep '$botToken' | cut -d"'" -f2)
telegramBotToken2=$(cat /var/www/html/v2bot/baseInfo.php | grep '$botToken' | cut -d'"' -f2)
filepath="/var/www/html/v2bot/baseInfo.php"
chatID=$(cat $filepath | grep '$admin =' | sed 's/.*= //' | sed 's/;//')

databaseUser=$(cat /var/www/html/v2bot/baseInfo.php | grep '$dbUserName' | cut -d"'" -f2)
databasePassword=$(cat /var/www/html/v2bot/baseInfo.php | grep '$dbPassword' | cut -d"'" -f2)
databaseName=$(cat /var/www/html/v2bot/baseInfo.php | grep '$dbName' | cut -d"'" -f2)
